...
NULLKEY = "-null-"
import simi.calc_similarity_word as cal_simi
    ...
    for idx in range(len(in_lines)):
        ...
        if len(line) > 2:
            ...
            if ((max_sent_length < 0) or (len(words) < max_sent_length)) and (len(words)>0):
                ...
                simi = cal_simi.calc_sim('TCM_corpus/dictionary_smptom_afterprocess.txt', simi_sentence,
                                         'TCM_corpus/word_sp.txt', 'TCM_corpus/simi/simitrain.txt')

                instence_texts.append([words, biwords, chars, gazs, labels])
                instence_Ids.append([word_Ids, biword_Ids, char_Ids, gaz_Ids, label_Ids, gazs, gazs_count, gaz_char_Id, layergazmasks,gazchar_masks, bert_text_ids,words,simi])
...
def build_pretrain_embedding(embedding_path, word_alphabet, embedd_dim=100, norm=True):
    """
    通过循环遍历，得到word_alphabet中每一个字在char_emb中的嵌入，不再char_emb中的字则随机赋值
    """
    embedd_dict = dict()
    ...
    pretrain_emb = np.empty([word_alphabet.size(), embedd_dim]) #初始化一个字表长度*嵌入纬度的向量
    perfect_match = 0
    case_match = 0
    not_match = 0
    pretrain_emb[0,:] = np.random.uniform(-scale, scale, [1, embedd_dim]) #初始化unkown的向量
    for word, index in word_alphabet.instance2index.items(): #字表中第一个为unknown
        if word in embedd_dict: #如果在字表中查到该字，则赋予嵌入值
            if norm:
                pretrain_emb[index,:] = norm2one(embedd_dict[word]) #vec/root_sum_square
            else:
...
def load_pretrain_emb(embedding_path):
    ...
    with open(embedding_path, 'r',encoding="utf-8") as file:
        for line in file:
            ...
            embedd = np.empty([1, embedd_dim])  #初始化一个向量，1*embedd_dim纬
            embedd[:] = tokens[1:] #将字嵌入中的向量赋值给embedd
            embedd_dict[tokens[0]] = embedd #生成一个嵌入字典
