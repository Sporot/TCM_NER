...
def evaluate(data, model, name):
    ...
    for batch_id in range(total_batch):
        with torch.no_grad():
            ...
            if not instance:
                continue
            gaz_list,batch_word, batch_biword, batch_wordlen, batch_label, layer_gaz, gaz_count, gaz_chars, gaz_mask, gazchar_mask, mask, batch_bert, bert_mask,simi_value  = batchify_with_label(instance, data.HP_gpu, data.HP_num_layer, True)
            tag_seq, gaz_match = model(gaz_list,batch_word, batch_biword, batch_wordlen, layer_gaz, gaz_count,gaz_chars, gaz_mask, gazchar_mask, mask, batch_bert, bert_mask,simi_value)
...
def batchify_with_label(input_batch_list, gpu, num_layer, volatile_flag=False):
    ...
    simi_value = [sent[12] for sent in input_batch_list]
    ...
    return gazs, word_seq_tensor, biword_seq_tensor, word_seq_lengths, label_seq_tensor, layer_gaz_tensor, gaz_count_tensor,gaz_chars_tensor, gaz_mask_tensor, gazchar_mask_tensor, mask, bert_seq_tensor, bert_mask,simi_value



def train(data, save_model_dir, seg=True):

    logging.info("Training with {} model.".format(data.model_type))
    ...
    best_test_p = -1
    best_test_r = -1
    logging.info('HP_iteration: '+str(data.HP_iteration))
    ...
    for idx in range(data.HP_iteration):
        ...
        right_token = 0
        whole_token = 0
        random.shuffle(data.train_Ids) #随机排序,将trains_id随机排序了
        ## set model in train model
        model.train()
        model.zero_grad()
        batch_size = data.HP_batch_size
        batch_id = 0
        train_num = len(data.train_Ids)  #训练集中的句子数目
        total_batch = train_num//batch_size+1
        ...
        for batch_id in range(total_batch):
            ...
            words = data.train_texts[start:end] #当前所选的训练集中完整的句子

            if not instance:
                continue

            gaz_list,  batch_word, batch_biword, batch_wordlen, batch_label, layer_gaz, gaz_count, \
            gaz_chars, gaz_mask, gazchar_mask, mask, batch_bert, bert_mask,simi_value = batchify_with_label(instance, data.HP_gpu,data.HP_num_layer)

            instance_count += 1
            #调用get_tags()
            loss, tag_seq = model.neg_log_likelihood_loss(gaz_list, batch_word, batch_biword, batch_wordlen, layer_gaz, gaz_count,gaz_chars, gaz_mask, gazchar_mask, mask, batch_label, batch_bert, bert_mask,simi_value)
        ...
        logging.info("Best dev score: p:{}, r:{}, f:{}".format(best_dev_p,best_dev_r,best_dev))
        logging.info("Test score: p:{}, r:{}, f:{}".format(best_test_p,best_test_r,best_test))
        gc.collect()
...
def load_model_decode(model_dir, data, name, gpu, seg=True):
    ...
    logging.info(("Decode %s data ..."%(name)))
    start_time = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelname', default="TCM-model-200-60-huo")
    parser.add_argument('--savedset', help='Dir of saved data setting', default="data/tcm_withbook-200-60-huo.dset")
    parser.add_argument('--train', default="data/TCM/train.txt")
    parser.add_argument('--dev', default="data/TCM/dev.txt")
    parser.add_argument('--test', default="data/TCM/test.txt")
    ...
    parser.add_argument('--num_iter',default=65,type=int)
    ...
    set_logger('save_model/train-tcm-mine-200-60-huo.log')
    ...
    char_emb = "TCM_corpus/vec/char_200_em.txt"
    #char_emb = "data/gigaword_chn.all.a2b.uni.ite50.vec"
    bichar_emb = "TCM_corpus/vec/bichar_200_em.txt"
    #bichar_emb = "data/gigaword_chn.all.a2b.bi.ite50.vec"
    gaz_file = "TCM_corpus/vec/word_200_em.txt"
    ...
    if status == 'train':
        if os.path.exists(save_data_name):
            ...
            with open(save_data_name, 'wb') as f:
                pickle.dump(data, f)  #存储训练好的data参数
            set_seed(seed_num)
        logging.info('data.use_biword='+str(data.use_bigram))
        #start training
